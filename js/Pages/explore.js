...
const configureLockageChildren = (index, hardware) => {
  return (index > currentUser.tech1.rung) ? [c("h1", {}), c("div", {"class": "lockCont"}, [c("img", {"src":"./res/img/lock.svg", "draggable": false})])] : [c("h1", {}, [hardware])];
};

const configureExplore = () => {
  techTree.forEach((row, i) => {
    document.getElementById("explorePage").appendChild(c("div", {"class": (i <= currentUser.tech1.rung) ? "techRow" : "techRow lockedTechRow"}, row.map(hardware => 
      c("div", {"class": "card", "style": `flex:1 1 0; background-image: url("./res/hardware/${hardware.split(/\s/g).join('_')}/topDown.${hardware.split(/\s/g).join('_')}.webp")`},
	configureLockageChildren(i, hardware),
	{
	  "click": () => {exploreActivity(hardware);}
	}
      ))
    ));
  });
...
const blurBott = (e) => {
  ...
  if (elem.scrollHeight - elem.scrollTop < elem.clientHeight*2) {
    elem.style.webkitMaskImage = "linear-gradient(black " + (100 - 25*(1-(elem.scrollHeight - elem.scrollTop - elem.clientHeight)/(elem.clientHeight))) + "%, transparent 100%)";
